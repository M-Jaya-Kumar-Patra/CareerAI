...
export function setAuthCookies(res, userId) {
  const accessToken = jwt.sign({ sub: userId.toString(), type: 'access' }, env.JWT_ACCESS_SECRET, { expiresIn: '15m' });
  ...
  return RefreshToken.create({ userId, tokenHash: hashToken(refreshToken), expiresAt: new Date(Date.now() + refreshMaxAge) });
}
...
export function clearAuthCookies(res) {
  res.clearCookie(ACCESS_COOKIE, { httpOnly: true, secure: env.NODE_ENV === 'production', sameSite: 'lax', path: '/' });
  res.clearCookie(REFRESH_COOKIE, { httpOnly: true, secure: env.NODE_ENV === 'production', sameSite: 'lax', path: '/' });
}
